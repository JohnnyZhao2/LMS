...
from apps.users.selectors import get_user_or_404, list_users
...
class UserListCreateView(APIView):
    ...
    @extend_schema(
        summary='获取用户列表',
        description='获取当前用户组织关系范围内的用户列表',
        parameters=[
    ...
    def get(self, request):
        get_engine(request).require_permission('users.view_user', error_message='无权查看用户列表')
        queryset = list_users(
        ...
        queryset = get_engine(request).scope_filter('users.view_user',
            resource_model=User,
    ...
    @extend_schema(
        summary='创建用户',
        description='创建新用户；管理角色可选，默认可学习',
    ...
    def post(self, request):
        get_engine(request).enforce('users.add_user', error_message='只有管理员可以创建用户')
        serializer = UserCreateSerializer(data=request.data, context={'request': request})
        serializer.is_valid(raise_exception=True)
        if serializer.validated_data.get('role_codes'):
            get_engine(request).require_permission('users.assign_user_role', error_message='无权分配用户角色')
...
class UserDetailView(APIView):
    ...
    def get(self, request, pk):
        ...
        get_engine(request).enforce('users.view_user',
            resource=user,
            error_message='无权查看该用户详情',
        )
    ...
    @extend_schema(
        summary='删除用户',
        description='彻底删除用户及其全部关联数据',
        responses={
            200: OpenApiResponse(description='删除成功'),
            400: OpenApiResponse(description='参数错误或用户仍被业务数据引用'),
            403: OpenApiResponse(description='无权限'),
            404: OpenApiResponse(description='用户不存在'),
        },
        tags=['用户管理'],
    ...
    def delete(self, request, pk):
        user = self.get_object(pk)
        get_engine(request).enforce('users.delete_user',
            resource=user,
            error_message='无权删除该用户',
        )
