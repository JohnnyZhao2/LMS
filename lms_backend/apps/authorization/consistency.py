"""Static authorization consistency checks."""
...
BACKEND_APPS_ROOT = REPO_ROOT / 'lms_backend' / 'apps'
FRONTEND_SRC_ROOT = REPO_ROOT / 'lms_frontend' / 'src'
...
TS_OBJECT_BLOCK_PATTERN = r"export const {name}:[^=]*=\s*\{{(.*?)\n\}};"
...
def _extract_module_presentation_codes() -> Set[str]:
...
def _iter_frontend_source_files():
    for pattern in ('*.ts', '*.tsx', '*.js', '*.jsx'):
        for path in FRONTEND_SRC_ROOT.rglob(pattern):
            yield path


def _extract_used_backend_permission_codes() -> Set[str]:
    codes: Set[str] = set()
...
def _extract_used_frontend_permission_codes() -> Set[str]:
    codes: Set[str] = set()
...
def _extract_used_permission_codes() -> Set[str]:
    # Frontend dotted strings are already almost entirely permission literals.
    # Backend只收敛到常见权限调用位，避免把 model field 路径误判成权限。
    return _extract_used_backend_permission_codes() | _extract_used_frontend_permission_codes()


def validate_authorization_consistency() -> List[str]:
    errors: List[str] = []
