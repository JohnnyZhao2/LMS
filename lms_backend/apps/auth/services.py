...
class AuthenticationService(BaseService):
    ...
    def get_one_account_authorize_url(self) -> dict[str, str]:
        return {
            'authorize_url': OneAccountClient().build_authorize_url(),
        }

    def login_by_one_account_code(self, code: str) -> dict[str, Any]:
        one_account_result = OneAccountClient().exchange_code(code=code)
    ...
    def change_my_password(
        ...
        return self._build_auth_payload(
            active_user,
            requested_role=getattr(user, 'current_role', None),
        )
