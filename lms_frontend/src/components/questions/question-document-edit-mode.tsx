...
export const QuestionDocumentEditMode: React.FC<QuestionDocumentEditModeProps> = ({
  ...
  const explanationTrigger = showExplanation ? (
    <Popover open={showExplanation && isExplanationPopoverOpen} onOpenChange={setIsExplanationPopoverOpen}>
  ...
  return (
    <div
      className={cn('@container overflow-hidden rounded-xl border border-border bg-background', className)}
    >
      {metaBar ? <div className="border-b border-border px-5 py-3">{metaBar}</div> : null}

      <div
        className="relative grid grid-cols-1 gap-4 px-5 py-3 @min-[700px]:grid-cols-[46%_minmax(0,1fr)] @min-[700px]:gap-0"
      >
        <div className="min-w-0 @min-[700px]:pr-5">
        ...
        <div className="min-w-0 @min-[700px]:pl-5">
          <div className="mb-2 text-[11px] font-semibold uppercase tracking-[0.14em] text-text-muted">
