...
const UserChip = ({
  ...
  user: Pick<UserList, 'avatar_key' | 'username' | 'department' | 'is_active'>;
  extra?: string;
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        {members.map((member) => (
                          <div
                            ...
                            onClick={() => {
                              if (canSelectMember && onSelectMember) {
                                onSelectMember(roleCode, member);
                              }
                            }}
                            onKeyDown={(event) => {
                              if (!canSelectMember || !onSelectMember) {
                            ...
                            className={cn(
                              ...
                              canSelectMember
                                ? 'cursor-pointer transition-colors duration-200 hover:bg-muted/35'
                                : '',
                            )}
                          ...
                          >
                            <UserChip
                              user={member}
                              extra={!member.is_active ? ' · 已停用' : undefined}
                              textClassName="pr-3"
                            />
                            {canManageMembers ? (
                              <button
                                ...
                                className="pointer-events-none absolute right-0 top-0 z-10 flex h-4.5 w-4.5 translate-x-[26%] -translate-y-[26%] items-center justify-center rounded-full border border-border/70 bg-white p-0 text-text-muted opacity-0 shadow-[0_1px_4px_rgba(15,23,42,0.08)] transition-all duration-150 group-hover:pointer-events-auto group-hover:opacity-100 hover:border-destructive/25 hover:text-destructive focus-visible:pointer-events-auto focus-visible:opacity-100 focus-visible:outline-none disabled:opacity-40"
                              >
