import React from 'react';
import { Loader2, Save, Trash2 } from 'lucide-react';

import { Button } from '@/components/ui/button';
import { normalizeQuestionTypeFields } from '@/features/assessment/components/questions/question-config';
import type { QuestionType, Tag } from '@/types/common';

import { QuestionDocumentEditMode } from '@/features/assessment/components/questions/question-document-edit-mode';
import { QuestionMetaToolbar } from '@/features/assessment/components/questions/question-meta-toolbar';
import type { AssessmentTagDeps } from '@/features/assessment/types/tag-deps';

export interface QuestionEditCardValue {
  questionId: number | null;
  questionType: QuestionType;
  spaceTagId?: number | null;
  content: string;
  options: Array<{ key: string; value: string }>;
  answer: string | string[];
  explanation: string;
  showExplanation: boolean;
  score: string;
  tagIds?: number[];
}

interface QuestionEditCardProps {
  item: QuestionEditCardValue;
  spaceTags?: Tag[];
  TagInput: AssessmentTagDeps['TagInput'];
  showScore?: boolean;
  lockQuestionType?: boolean;
  leadingSlot?: React.ReactNode;
  onChange: (patch: Partial<QuestionEditCardValue>) => void;
  onFocus?: () => void;
  onDelete?: () => void;
  onSave?: () => void;
  isSaving?: boolean;
  isDeleting?: boolean;
  showMetaToolbar?: boolean;
  headerActions?: React.ReactNode;
}

export const QuestionEditCard: React.FC<QuestionEditCardProps> = ({
  item,
  spaceTags,
  TagInput,
  showScore = false,
  lockQuestionType,
  leadingSlot,
  onChange,
  onFocus,
  onDelete,
  onSave,
  isSaving = false,
  isDeleting = false,
  showMetaToolbar = false,
  headerActions,
}) => {
  const defaultFooterActions = (onDelete || onSave) ? (
    <div className="flex items-center gap-2.5">
      {onDelete ? (
        <Button
          variant="ghost"
          size="icon"
          className="h-8 w-8 rounded-none border-none bg-transparent p-0 text-destructive shadow-none hover:bg-transparent hover:text-destructive/85"
          onClick={(event) => {
            event.stopPropagation();
            onDelete();
          }}
          disabled={isDeleting || isSaving}
          aria-label="删除"
          title="删除"
        >
          {isDeleting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Trash2 className="h-4 w-4" />}
        </Button>
      ) : null}
      {onSave ? (
        <Button
          variant="ghost"
          size="icon"
          className="h-8 w-8 rounded-none border-none bg-transparent p-0 text-primary-600 shadow-none hover:bg-transparent hover:text-primary-500"
          onClick={(event) => {
            event.stopPropagation();
            onSave();
          }}
          disabled={isDeleting || isSaving}
          aria-label="保存"
          title="保存"
        >
          {isSaving ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />}
        </Button>
      ) : null}
    </div>
  ) : null;

  return (
    <div className="group flex items-start gap-2.5 transition-all">
      {leadingSlot}

      <div onClick={onFocus} className="min-w-0 flex-1">
        <QuestionDocumentEditMode
          questionType={item.questionType}
          content={item.content}
          options={item.options}
          answer={item.answer}
          explanation={item.explanation}
          showExplanation={item.showExplanation}
          onContentChange={(value) => onChange({ content: value })}
          onOptionsChange={(value) => onChange({ options: value })}
          onAnswerChange={(value) => onChange({ answer: value })}
          onExplanationChange={(value) => onChange({ explanation: value })}
          onShowExplanationChange={(show) => {
            if (!show) {
              onChange({ showExplanation: false, explanation: '' });
              return;
            }
            onChange({ showExplanation: true });
          }}
          metaBar={showMetaToolbar ? (
            <QuestionMetaToolbar
              questionType={item.questionType}
              staticType={lockQuestionType ?? !!item.questionId}
              onQuestionTypeChange={(value) => {
                const normalized = normalizeQuestionTypeFields({
                  options: item.options,
                  answer: item.answer,
                }, value);

                onChange({
                  questionType: value,
                  options: normalized.options ?? item.options,
                  answer: normalized.answer ?? item.answer,
                });
              }}
              score={item.score}
              onScoreChange={(value) => onChange({ score: value })}
              showType
              showScore={showScore}
              showSpace
              showTags
              spaceTags={spaceTags}
              spaceTagId={item.spaceTagId}
              selectedTagIds={item.tagIds ?? []}
              onSpaceTagIdChange={(value) => onChange({ spaceTagId: value })}
              onTagAdd={(tag) => onChange({
                tagIds: [...new Set([...(item.tagIds ?? []), tag.id])],
              })}
              onTagRemove={(tagId) => onChange({
                tagIds: (item.tagIds ?? []).filter((id) => id !== tagId),
              })}
              TagInput={TagInput}
            />
          ) : null}
          footerActions={(headerActions || defaultFooterActions) ? (
            <div className="flex items-center gap-2.5">
              {headerActions}
              {defaultFooterActions}
            </div>
          ) : null}
        />
      </div>
    </div>
  );
};
