import { ensureChoiceOptions } from '@/config/questions';
...
export const buildQuestionPatchPayload = (
  ...
  return Object.fromEntries(
    fields
      .filter((field) => JSON.stringify(normalizeCompareValue(baseline[field], field)) !== JSON.stringify(normalizeCompareValue(current[field], field)))
      .map((field) => [field, current[field]]),
  ) as Partial<QuestionCreateRequest>;
};
