# 组件清单

> 自动生成文件。请勿手改；执行 `npm run docs:generate` 更新。

- 共享组件：49 个
- Feature 作用域视觉组件：107 个

## 共享组件

### common (5 个)

- `src/components/common/icon-box.tsx`
- `src/components/common/micro-label.tsx`
- `src/components/common/selection-indicator.tsx`
- `src/components/common/user-avatar.tsx`
- `src/components/common/user-selectable-list.tsx`

### ui (44 个)

- `src/components/ui/action-card.tsx`
- `src/components/ui/alert.tsx`
- `src/components/ui/avatar.tsx`
- `src/components/ui/badge.tsx`
- `src/components/ui/breadcrumb-nav.tsx`
- `src/components/ui/button.tsx`
- `src/components/ui/calendar.tsx`
- `src/components/ui/card.tsx`
- `src/components/ui/checkbox.tsx`
- `src/components/ui/circle-button.tsx`
- `src/components/ui/compact-number-input.tsx`
- `src/components/ui/confirm-dialog.tsx`
- `src/components/ui/data-table/data-table-cells.tsx`
- `src/components/ui/data-table/data-table.tsx`
- `src/components/ui/date-picker.tsx`
- `src/components/ui/dialog.tsx`
- `src/components/ui/dropdown-menu.tsx`
- `src/components/ui/empty-state.tsx`
- `src/components/ui/error-boundary.tsx`
- `src/components/ui/form.tsx`
- `src/components/ui/input.tsx`
- `src/components/ui/label.tsx`
- `src/components/ui/list-tag.tsx`
- `src/components/ui/page-header.tsx`
- `src/components/ui/page-shell.tsx`
- `src/components/ui/pagination.tsx`
- `src/components/ui/popover.tsx`
- `src/components/ui/progress.tsx`
- `src/components/ui/route-skeleton.tsx`
- `src/components/ui/scroll-container.tsx`
- `src/components/ui/search-input.tsx`
- `src/components/ui/segmented-control.tsx`
- `src/components/ui/select.tsx`
- `src/components/ui/selectable-list.tsx`
- `src/components/ui/separator.tsx`
- `src/components/ui/skeleton.tsx`
- `src/components/ui/sonner.tsx`
- `src/components/ui/spinner.tsx`
- `src/components/ui/stat-card.tsx`
- `src/components/ui/table.tsx`
- `src/components/ui/tabs.tsx`
- `src/components/ui/textarea.tsx`
- `src/components/ui/toggle-switch.tsx`
- `src/components/ui/tooltip.tsx`

## Feature 作用域视觉组件

### activity-logs (5 个)

#### components

- `src/features/activity-logs/components/activity-log-feed.tsx`
- `src/features/activity-logs/components/activity-log-member-list.tsx`
- `src/features/activity-logs/components/activity-log-policy-category-section.tsx`
- `src/features/activity-logs/components/activity-log-policy-panel.tsx`
- `src/features/activity-logs/components/activity-logs-panel.tsx`

### assessment (36 个)

#### components

- `src/features/assessment/components/grading/grading-center-page.tsx`
- `src/features/assessment/components/grading/grading-center-tab.tsx`
- `src/features/assessment/components/questions/question-add-menu.tsx`
- `src/features/assessment/components/questions/question-bank-panel.tsx`
- `src/features/assessment/components/questions/question-bank-picker.tsx`
- `src/features/assessment/components/questions/question-batch-editor.tsx`
- `src/features/assessment/components/questions/question-detail-dialog.tsx`
- `src/features/assessment/components/questions/question-detail-preview.tsx`
- `src/features/assessment/components/questions/question-document-answer-panels.tsx`
- `src/features/assessment/components/questions/question-document-edit-mode.tsx`
- `src/features/assessment/components/questions/question-document-list.tsx`
- `src/features/assessment/components/questions/question-document-read-mode.tsx`
- `src/features/assessment/components/questions/question-document-shared.tsx`
- `src/features/assessment/components/questions/question-edit-card.tsx`
- `src/features/assessment/components/questions/question-form-inputs.tsx`
- `src/features/assessment/components/questions/question-form-page.tsx`
- `src/features/assessment/components/questions/question-management-page.tsx`
- `src/features/assessment/components/questions/question-meta-toolbar.tsx`
- `src/features/assessment/components/questions/question-tab.tsx`
- `src/features/assessment/components/questions/question-type-badge.tsx`
- `src/features/assessment/components/questions/sortable-option-item.tsx`
- `src/features/assessment/components/quizzes/quiz-document-editor.tsx`
- `src/features/assessment/components/quizzes/quiz-form.tsx`
- `src/features/assessment/components/quizzes/quiz-management-page.tsx`
- `src/features/assessment/components/quizzes/quiz-outline-panel.tsx`
- `src/features/assessment/components/quizzes/quiz-preview-dialog.tsx`
- `src/features/assessment/components/quizzes/quiz-preview-workbench.tsx`
- `src/features/assessment/components/quizzes/quiz-tab.tsx`
- `src/features/assessment/components/quizzes/sortable-outline-item.tsx`
- `src/features/assessment/components/submissions/answer-review.tsx`
- `src/features/assessment/components/submissions/question-card.tsx`
- `src/features/assessment/components/submissions/quiz-player-dialogs.tsx`
- `src/features/assessment/components/submissions/quiz-player-main-panel.tsx`
- `src/features/assessment/components/submissions/quiz-player-panels.tsx`
- `src/features/assessment/components/submissions/quiz-player.tsx`
- `src/features/assessment/components/submissions/timer.tsx`

### auth (3 个)

#### components

- `src/features/auth/components/change-own-password-dialog.tsx`
- `src/features/auth/components/login-form.tsx`
- `src/features/auth/components/role-switch-overlay.tsx`

### dashboard (9 个)

#### components

- `src/features/dashboard/components/admin-dashboard.tsx`
- `src/features/dashboard/components/exam-report-export-dialog.tsx`
- `src/features/dashboard/components/exam-report-panel.tsx`
- `src/features/dashboard/components/mentor-dashboard.tsx`
- `src/features/dashboard/components/student-dashboard.tsx`
- `src/features/dashboard/components/student-dashboard/editorial-card.tsx`
- `src/features/dashboard/components/student-dashboard/items.tsx`
- `src/features/dashboard/components/student-dashboard/mini-calendar.tsx`
- `src/features/dashboard/components/team-manager-dashboard.tsx`

### knowledge (15 个)

#### components

- `src/features/knowledge/components/cards/knowledge-add-card.tsx`
- `src/features/knowledge/components/cards/knowledge-card.tsx`
- `src/features/knowledge/components/editor/format-toolbar.tsx`
- `src/features/knowledge/components/editor/knowledge-text-editor.tsx`
- `src/features/knowledge/components/editor/slash-menu.tsx`
- `src/features/knowledge/components/knowledge-center.tsx`
- `src/features/knowledge/components/knowledge-masonry-grid.tsx`
- `src/features/knowledge/components/modals/knowledge-detail-modal.tsx`
- `src/features/knowledge/components/modals/knowledge-detail-outline.tsx`
- `src/features/knowledge/components/modals/knowledge-detail-side-panel.tsx`
- `src/features/knowledge/components/modals/knowledge-learning-action.tsx`
- `src/features/knowledge/components/shared/focus-icon.tsx`
- `src/features/knowledge/components/shared/knowledge-action-button.tsx`
- `src/features/knowledge/components/shared/knowledge-content-renderer.tsx`
- `src/features/knowledge/components/shared/related-links-editor.tsx`

### spot-checks (7 个)

#### components

- `src/features/spot-checks/components/spot-check-form.tsx`
- `src/features/spot-checks/components/spot-check-image-gallery.tsx`
- `src/features/spot-checks/components/spot-check-item-editor.tsx`
- `src/features/spot-checks/components/spot-check-list.tsx`
- `src/features/spot-checks/components/spot-check-record-list.tsx`
- `src/features/spot-checks/components/spot-check-student-panel.tsx`
- `src/features/spot-checks/components/student-spot-check-panel.tsx`

### tags (7 个)

#### components

- `src/features/tags/components/sortable-space-tag-item.tsx`
- `src/features/tags/components/space-color-ring-picker.tsx`
- `src/features/tags/components/space-tag-quick-create-dialog.tsx`
- `src/features/tags/components/tag-assignment-section.tsx`
- `src/features/tags/components/tag-form-dialog.tsx`
- `src/features/tags/components/tag-input.tsx`
- `src/features/tags/components/tag-management-page.tsx`

### tasks (11 个)

#### components

- `src/features/tasks/components/student-task-list.tsx`
- `src/features/tasks/components/task-card.tsx`
- `src/features/tasks/components/task-detail.tsx`
- `src/features/tasks/components/task-form/sortable-resource-item.tsx`
- `src/features/tasks/components/task-form/task-configuration-panel.tsx`
- `src/features/tasks/components/task-form/task-form.tsx`
- `src/features/tasks/components/task-form/task-pipeline-panel.tsx`
- `src/features/tasks/components/task-form/task-resource-library-panel.tsx`
- `src/features/tasks/components/task-management.tsx`
- `src/features/tasks/components/task-preview/progress-monitoring-tab.tsx`
- `src/features/tasks/components/task-preview/task-preview-page.tsx`

### user-management (14 个)

#### components

- `src/features/user-management/components/authorization/authorization-center-page.tsx`
- `src/features/user-management/components/authorization/permission-module-sections.tsx`
- `src/features/user-management/components/authorization/permission-toggle-card.tsx`
- `src/features/user-management/components/authorization/role-permission-template-panel.tsx`
- `src/features/user-management/components/authorization/role-template-member-panel.tsx`
- `src/features/user-management/components/authorization/user-permission-module-list.tsx`
- `src/features/user-management/components/authorization/user-permission-scope-popover.tsx`
- `src/features/user-management/components/authorization/user-permission-section.tsx`
- `src/features/user-management/components/authorization/user-permission-workbench.tsx`
- `src/features/user-management/components/authorization/user-role-assignment-chips.tsx`
- `src/features/user-management/components/users/avatar-picker-popover.tsx`
- `src/features/user-management/components/users/user-directory-filters.tsx`
- `src/features/user-management/components/users/user-form.tsx`
- `src/features/user-management/components/users/user-list.tsx`
